      ...
      if (config === undefined || !config.disabled) {
        ...
        if (strategy === EnvStrategyType.STRATEGY_LOCAL) {
          ...
          if (isSingleSignOnInGuiEnabled) {
            ...
            registerAuthenticationProvider(LOCAL_STRATEGY_IDENTIFIER, localStrategy,
              { name: providerName, type: AuthType.AUTH_FORM, strategy, provider: LOCAL_STRATEGY_IDENTIFIER });
          }
        }
        ...
        if (strategy === EnvStrategyType.STRATEGY_OPENID) {
          ...
          enrichWithRemoteCredentials(`providers:${providerIdent}`, config).then((clientConfig) => {
            OpenIDIssuer.discover(config.issuer).then((issuer) => {
              ...
              const options = {
                logout_remote: mappedConfig.logout_remote, client, passReqToCallback: true,
                params: {
                  scope: openIdScope, ...(mappedConfig.audience && { audience: mappedConfig.audience }),
                },
              };
              ...
              logApp.error('[ENV-PROVIDER][OPENID] Error initializing authentication provider', {
                cause: err,
