...
import { EVENT_TYPE_CREATE, EVENT_TYPE_DELETE, isEmptyField, UPDATE_OPERATION_ADD, UPDATE_OPERATION_REMOVE } from './utils';
...
const humanizeRawValue = async (resolvedMap: Record<string, string>,
  ...
  if (attribute?.type === 'string') {
    if (attribute.format === 'id' || attribute.format === 'json') {
      return attribute.representative?.(v.raw, resolvedMap, format) ?? 'Restricted';
    }
  ...
  if (attribute?.type === 'object') {
    try {
      ...
      return attribute.representative?.(objectData, resolvedMap, format) ?? 'Restricted';
    } catch {
      return 'Restricted';
    }
  }
  if (attribute?.type === 'ref') {
    return resolvedMap[v.raw] ?? 'Restricted';
  }
  return 'Restricted';
};

const humanizeChangeValues = async (resolvedMap: Record<string, string>,
  field: string, changeValues: ChangeValue[] | undefined, format: Formating = DefaultFormating): Promise<HumanChange[]> => {
  const attribute = resolveAttribute(field);
  const humanChangedValues: HumanChange[] = [];
  const values = changeValues ?? [];
  for (let index = 0; index < values.length; index++) {
    const v = values[index];
    const human = attribute ? await humanizeRawValue(resolvedMap, attribute, v, format) : 'Restricted';
    humanChangedValues.push({ raw: v.raw, human });
  }
...
export const buildChanges = async (context: AuthContext, user: AuthUser,
  ...
  const inputsChanges = inputs.flatMap((input) => {
    ...
    const changes_added = [...prev, ...next].map((item) => {
      const data = typeof item === 'object' ? JSON.stringify(item) : item;
      return ({ raw: data });
    });
    return { field: entityType + '--' + field, changes_added };
  });
  const resolvedMap = inputsChanges.length > 0
    ? await attributesChangesResolver(context, user, [inputsChanges]) : {};
